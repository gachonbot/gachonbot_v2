...
let snuWeatherScheduler = schedule.scheduleJob('5 * * * *', function(){
  rp(`https://api2.sktelecom.com/weather/current/hourly?version=1&lat=37.460079&lon=126.952327&appkey=${process.env.WEATHER_KEY}`)
      .then(function (response) {
          response = JSON.parse(response);
          response = response.weather.hourly[0];
          models.Weather.create({
...
let snuAirScheduler = schedule.scheduleJob('5 * * * *', function(){
  rp(`http://openapi.airkorea.or.kr/openapi/services/rest/ArpltnInforInqireSvc/getMsrstnAcctoRltmMesureDnsty?serviceKey=${process.env.AIR_KEY}&numOfRows=1&pageSize=1&pageNo=1&startPage=1&stationName=${encodeURIComponent('관악구')}&dataTerm=DAILY&ver=1.3&_returnType=json`)
      .then(function (response) {
        response = JSON.parse(response);
        response = response.list[0];
        models.Air.create({
